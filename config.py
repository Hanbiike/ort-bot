...
SUBJECT_FILES = {
    ...
    "Аналогии": "subjects/analogy.json",
...
SUBJECT_TASK_STYLES = {
    ...
    "Аналогии": ["Смешной", "Строгий", "Сюжетный", "Максимальная креативность"],
    "Химия": ["Строгий"],
    "Биология": ["Строгий"]
}
...
Правильный ответ: В
    ...
    "Аналогии" : {
        "ABCDE": """
Ты — генератор тестовых заданий для подготовки к Общереспубликанскому тестированию (ОРТ) в Кыргызстане, который похож на SAT. 
Сгенерируй ОДНУ аналогию в формате теста. В тесте 5 варианта ответа: А, Б, В, Г, Д, где лишь один вариант правильный.

Правила:
1. Каждая аналогия в тесте состоит из 6 пар слов. Первая пара называется «эталонной парой». Эталонная пара задает, показывает образец определенного типа связи между словами.
...
1. Легкий: в одной из пар слов может встретиться ловушка - пара с верным типом связи, но с неверным порядком слов в паре. Следи, чтобы только ОДИН вариант оставался строго корректным.
2. Средний: все ловушки из легкого уровня + как минимум ОДНА логическая подмена уровня: часть—целое vs род—вид; причина—следствие vs средство—цель; объект—место vs место—житель. Следи, чтобы только ОДИН вариант оставался строго корректным.
3. Сложный: все ловушки из легкого и среднего уровней + ловушки строятся на тонких различиях (направление, уровень обобщения, необходимость/достаточность условия), а также возможно, что для решения нужно использовать несколько типов связи. Следи, чтобы только ОДИН вариант оставался строго корректным.
"""
    },
    "Химия": {
        "ABCDE": """
